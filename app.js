...
function initializeNutritionCalculator() {
    console.log('Initializing nutrition calculator...');
    
    // Set up form elements
    const ageInput = document.getElementById('age');
    const activitySelect = document.getElementById('activity');
    const calculateButton = document.querySelector('.nutrition-calculator .btn-primary');
    
    if (ageInput) {
        // Make sure age input is properly configured
        ageInput.addEventListener('input', function() {
            console.log('Age input changed:', this.value);
        });
        
        ageInput.addEventListener('focus', function() {
            this.style.borderColor = 'var(--saffola-red)';
        });
        
        ageInput.addEventListener('blur', function() {
            this.style.borderColor = 'var(--saffola-cream)';
        });
    }
    
    if (activitySelect) {
        // Make sure activity select is properly configured
        activitySelect.addEventListener('change', function() {
            console.log('Activity level changed:', this.value);
        });
...
function calculateNutrition() {
    ...
    if (!ageInput || !activitySelect) {
        console.error('Form elements not found');
        alert('Form elements not available. Please refresh the page and try again.');
        return;
    }
    ...
    console.log('Form values - Age:', age, 'Activity:', activity);
    ...
    if (ageNum < 5 || ageNum > 100) {
        alert('Please enter a valid age between 5 and 100 years.');
        ageInput.focus();
        return;
    }
    
    let proteinNeed, calorieNeed, servingRecommendation, nutritionTips;
    
    // Enhanced calculation logic based on age and activity
    if (activity === 'low') {
        proteinNeed = Math.round(ageNum < 18 ? 45 : ageNum < 30 ? 50 : ageNum < 50 ? 55 : 60);
        calorieNeed = Math.round(ageNum < 18 ? 1800 : ageNum < 30 ? 2000 : ageNum < 50 ? 1900 : 1800);
        nutritionTips = 'Focus on balanced nutrition and consider light physical activity.';
    } else if (activity === 'moderate') {
        proteinNeed = Math.round(ageNum < 18 ? 55 : ageNum < 30 ? 65 : ageNum < 50 ? 70 : 65);
        calorieNeed = Math.round(ageNum < 18 ? 2100 : ageNum < 30 ? 2300 : ageNum < 50 ? 2200 : 2000);
        nutritionTips = 'Great balance of activity! Maintain consistent protein intake for muscle recovery.';
    } else {
        proteinNeed = Math.round(ageNum < 18 ? 65 : ageNum < 30 ? 80 : ageNum < 50 ? 85 : 80);
        calorieNeed = Math.round(ageNum < 18 ? 2400 : ageNum < 30 ? 2600 : ageNum < 50 ? 2500 : 2300);
        nutritionTips = 'High activity level! Ensure adequate protein for optimal performance and recovery.';
    }
    
    servingRecommendation = Math.ceil(proteinNeed / 12); // 12g protein per serving
    const proteinContribution = Math.round((12 * servingRecommendation / proteinNeed) * 100);
    
    console.log('Calculated values:', { proteinNeed, calorieNeed, servingRecommendation, proteinContribution });
    ...
    if (recommendationsDiv) {
        recommendationsDiv.innerHTML = `
            ...
            <div class="recommendation-item">
                ...
                <span class="recommendation-value">${calorieNeed.toLocaleString()}</span>
            </div>
            <div class="recommendation-item">
            ...
            <div class="recommendation-item">
                ...
                <span class="recommendation-value">${proteinContribution}% of protein needs</span>
            </div>
        `;
        ...
        tipDiv.style.backgroundColor = 'var(--saffola-light-cream)';
        tipDiv.style.borderRadius = '12px';
        tipDiv.style.borderLeft = '4px solid var(--saffola-yellow)';
        tipDiv.innerHTML = `<strong>Nutrition Tip:</strong> ${nutritionTips}`;
        recommendationsDiv.appendChild(tipDiv);
    }
    ...
    console.log('Nutrition calculation completed successfully');
}
...
console.log('🚀 Ready for the best user experience!');
